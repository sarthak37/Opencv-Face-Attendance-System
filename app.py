...
## A function to delete a user's attendance records
def delete_attendance_records(userid):
    df = pd.read_csv(f'Attendance/Attendance-{datetoday}.csv')
...
def deleteuser():
    ...
    if duser is not None:
        ...
        delete_attendance_records(userid)
...
def listusers():
    ...
    users = zip(names, rolls)
    return render_template('listusers.html', users=users, l=l, totalreg=totalreg(), datetoday2=datetoday2)
...
def start():
    ...
    while ret:
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):  # Use 'q' to exit
            break
    cap.release()
